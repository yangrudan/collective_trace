...
def hook_batch_isend_irecv():
    ...
    def wrapped_batch_isend_irecv(ops_list):
        ...
        print(f"  总数据: {send_total + recv_total} 字节 ({(send_mb + recv_mb):.2f} MB)")
...
def main():
    ...
    for i in range(256):
        ...
        send_tensor.append(torch.ones((1280, 1280), dtype=torch.float32, device=f'cuda:{local_rank}') * rank + i * 0.001)
        recv_tensor.append(torch.randn((1280, 1280), dtype=torch.float32, device=f'cuda:{local_rank}'))
