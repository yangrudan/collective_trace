...
class TimedWork:
    ...
    def wait(self):
        ...
        print(f"Rank {dist.get_rank()}: {self.func_name} | 数据量[异步模式]: {self.data_size/1e6:.2f}MB | 耗时: {duration:.2f}ms")
        return result
...
def trace_async_collective(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if is_async:
            ...
            print(f"Rank {dist.get_rank()}: {func.__name__} | 数据量[同步模式]: {data_size/1e6:.2f}MB | 耗时: {duration:.2f}ms")
            return work
    return wrapper
