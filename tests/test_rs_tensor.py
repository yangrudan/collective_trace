...
def main():
    ...
    # 最终每个进程会收到归约后对应位置的元素
    input_tensor = torch.tensor([rank * world_size + i for i in range(world_size)], 
                               dtype=torch.float32, device=device)
    print(f"Rank {rank} 输入张量: {input_tensor.cpu().numpy()}")
    
    # 输出张量大小应为输入张量大小 / world_size
    output_tensor = torch.empty(input_tensor.numel() // world_size, 
                               dtype=torch.float32, device=device)
    ...
    if args.sync_mode:
        ...
        work = dist.reduce_scatter_tensor(output_tensor, input_tensor, 
