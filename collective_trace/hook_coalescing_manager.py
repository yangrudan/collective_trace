...
def _create_old_version_manager(origin_coalescing_manager):
    """Create timed coalescing manager for PyTorch <=2.0.1"""
    @contextmanager
    ...
    return timed_coalescing_manager


def _create_new_version_manager(origin_coalescing_manager):
    """Create timed coalescing manager for PyTorch >=2.0.2"""
    @contextmanager
    def timed_coalescing_manager(
        group: Optional[dist.ProcessGroup] = None,
        device: Optional[torch.device] = None,
...
def hook_coalescing_manager():
    ...
    if IS_OLD_VERSION:
        ...
        timed_manager = _create_old_version_manager(origin_coalescing_manager)
    else:
        origin_coalescing_manager = dist._coalescing_manager
        timed_manager = _create_new_version_manager(origin_coalescing_manager)

    if not IS_OLD_VERSION:
        # pylint: disable=protected-access
        dist._coalescing_manager = timed_manager
