...
class OperationTimer:
                ...
                for op_id, (start_time, func_name, is_async, is_completed) in list(
                    ...
                    if not is_completed:
                        if current_time - start_time > self.timeout_threshold:
                            ...
                            # callback
                            self.callback(
                                op_id, func_name, is_async, "unfinished")
    ...
    def register_operation(self, op_id, func_name, is_async):
        ...
        with self.lock:
            self.pending_ops[op_id] = (
                time.perf_counter(), func_name, is_async, False)
    ...
    def is_timed_out(self, op_id):
        ...
        with self.lock:
            ...
            return not is_completed and (
                time.perf_counter() -
                start_time > self.timeout_threshold)
...
class CollectiveTracer:
    ...
    def __init__(self, trace_file=None, verbose=True, timeout_threshold=None):
        ...
        self.call_counts = defaultdict(
            lambda: defaultdict(lambda: {"count": 0}))
    ...
    def _timeout_callback(self, op_id, func_name, is_async, timed_out_type):
        ...
        if timed_out_type == "unfinished":
            msg = f"[TIMEOUT] {async_str} Operation {func_name} (ID: {op_id}) \
                  timed out without completion! Exceeded threshold {self.timeout_threshold}s"
        else:
            msg = f"[TIMEOUT] {async_str} Operation {func_name} (ID: {op_id}) \
                  completed but timed out! Exceeded threshold {self.timeout_threshold}s"
        ...
        class TimedWork:
            ...
            def __init__(self, work, op_id, start_time, func_name, **kwargs):
                ...
                self.tensor_info = kwargs.get(
                    "tensor_info", {
                        "shape": "unknown", "dtype": "unknown", "size": 0})
    ...
    def hook_batch_isend_irecv(self):
        ...
        def wrapped_batch_isend_irecv(ops_list):
            ...
            send_targets_str = (", ".join(
                map(str, stats["send_targets"])) if stats["send_targets"] else "Null")
            recv_sources_str = (", ".join(
                map(str, stats["recv_sources"])) if stats["recv_sources"] else "Null")
            send_shapes_str = (", ".join(
                map(str, stats["send_shapes"])) if stats["send_shapes"] else "Null")
            recv_shapes_str = (", ".join(
                map(str, stats["recv_shapes"])) if stats["recv_shapes"] else "Null")
    ...
    def _trace_barrier(self, original_barrier):
        ...
        def barrier_traced(*args, **kwargs):
            ...
            trace_entry = self.create_trace_entry(
                "barrier", start_time, duration, None)
            self.trace_data.append(trace_entry)
    ...
    def remove_hooks(self):
        ...
        if "batch_isend_irecv" in self.original_functions:
            setattr(
                dist,
                "batch_isend_irecv",
                self.original_functions["batch_isend_irecv"])
